...
internal class WorkspaceConfigFetcherTest {
    ...
    fun `when exception on http call then throws that exception`() {
        ...
        // when
        val sut = WorkspaceConfigFetcher(HackleConfig.DEFAULT, sdk("test_key"), httpClient)
    ...
    fun `get url`() {
        ...
        // when
        val sut = WorkspaceConfigFetcher(config, sdk("SDK_KEY"), httpClient)
        val actual = sut.fetchIfModified()
    ...
    fun `last modified`() {
        ...
        val sut = WorkspaceConfigFetcher(HackleConfig.DEFAULT, sdk("SDK_KEY"), httpClient)
