...
internal class HttpWorkspaceConfigFetcherTest {
    ...
    fun `when exception on http call then throws that exception`() {
        ...
        // when
        val sut = HttpWorkspaceConfigFetcher(HackleConfig.DEFAULT, sdk("test_key"), httpClient)
    ...
    fun `get url`() {
        ...
        // when
        val sut = HttpWorkspaceConfigFetcher(config, sdk("SDK_KEY"), httpClient)
        val actual = sut.fetchIfModified()
    ...
    fun `last modified`() {
        ...
        val sut = HttpWorkspaceConfigFetcher(HackleConfig.DEFAULT, sdk("SDK_KEY"), httpClient)
